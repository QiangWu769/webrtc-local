...
FrameGeneratorInterface::VideoFrameData YuvFileGenerator::NextFrame() {
  ...
  if (current_display_count_ == 0) {
    ...
    if (got_new_frame) {
      ...
      // File ended, return null buffer to signal completion
      RTC_LOG(LS_INFO) << "Video file transmission completed, signaling end";
      return VideoFrameData(nullptr, update_rect);
    }
  }
...
bool YuvFileGenerator::ReadNextFrame() {
  ...
  auto new_buffer = test::ReadI420Buffer(
      static_cast<int>(width_), static_cast<int>(height_), files_[file_index_]);
  
  if (!new_buffer) {
    // End of file reached - do not loop, just keep using last frame
    RTC_LOG(LS_INFO) << "End of video file reached at frame " << frame_index_ 
                     << ", will hold last frame";
    return false;  // Signal end of stream, but keep last_read_buffer_ valid
  }
  
  // Update to new frame
  last_read_buffer_ = new_buffer;
  ++frame_index_;
