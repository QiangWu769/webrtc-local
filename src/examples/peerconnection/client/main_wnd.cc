...
void MainWnd::StartLocalRenderer(webrtc::VideoTrackInterface* local_video) {
  local_renderer_.reset(new VideoRenderer(this, handle(), 1, 1, local_video));
}
...
void MainWnd::StartRemoteRenderer(webrtc::VideoTrackInterface* remote_video) {
  remote_renderer_.reset(new VideoRenderer(this, handle(), 1, 1, remote_video));
}
...
MainWnd::VideoRenderer::VideoRenderer(
    ...
    : main_wnd_(main_wnd), wnd_(wnd), rendered_track_(track_to_render) {
...
void MainWnd::VideoRenderer::OnFrame(const webrtc::VideoFrame& video_frame) {
  // Professional video frame saving callback (same as AlphaRTC)
