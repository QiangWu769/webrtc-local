...
PeerConnectionClient::PeerConnectionClient()
    : callback_(nullptr),
      ...
      my_id_(-1),
      hanging_get_reconnect_scheduled_(false) {}
...
void PeerConnectionClient::OnClose(webrtc::Socket* socket, int err) {
  ...
  if (err != ECONNREFUSED) {
    ...
    if (socket == hanging_get_.get()) {
      if (state_ == CONNECTED && !hanging_get_reconnect_scheduled_) {
        hanging_get_->Close();
        hanging_get_reconnect_scheduled_ = true;
        // Add delay to prevent rapid reconnection storms
        RTC_LOG(LS_INFO) << "Hanging GET connection closed; retrying in 2 seconds";
        webrtc::Thread::Current()->PostDelayedTask(
            SafeTask(safety_.flag(), [this] { 
              hanging_get_reconnect_scheduled_ = false;
              if (state_ == CONNECTED && hanging_get_->GetState() == webrtc::Socket::CS_CLOSED) {
                hanging_get_->Connect(server_address_);
              }
            }), kReconnectDelay);
      }
