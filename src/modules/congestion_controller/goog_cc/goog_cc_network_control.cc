...
std::string GetWallClockTimestampString() {
 ...
 NetworkControlUpdate GoogCcNetworkController::OnTransportPacketsFeedback(
   ...
   // ============= GCC 完整决策快照日志 =============
   // 获取各组件状态信息
   std::string delay_state_str = "Normal";
   if (result.delay_detector_state == BandwidthUsage::kBwOverusing) {
     delay_state_str = "Overusing";
   } else if (result.delay_detector_state == BandwidthUsage::kBwUnderusing) {
     delay_state_str = "Underusing";
   }
   
   DataRate delay_target = result.updated ? result.target_bitrate : DataRate::Zero();
   DataRate bandwidth_target = bandwidth_estimation_->target_rate();
   bool rtt_backoff_active = bandwidth_estimation_->IsRttAboveLimit();
   DataRate probe_result = probe_bitrate.value_or(DataRate::Zero());
   DataRate acked_bitrate = acknowledged_bitrate.value_or(DataRate::Zero());
   
   // 确定决策原因
   std::string decision_reason = "Hold";
   DataRate final_target = DataRate::Zero();
   
   if (update.target_rate) {
     final_target = update.target_rate->target_rate;
     
     if (result.delay_detector_state == BandwidthUsage::kBwOverusing) {
       decision_reason = "DelayLimit";
     } else if (rtt_backoff_active) {
       decision_reason = "RttBackoff"; 
     } else if (result.probe && probe_bitrate.has_value()) {
       decision_reason = "ProbeResult";
     } else {
       decision_reason = "LossEstimate";
     }
   }
   ...
   RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "]" << " [GCC-DECISION-SNAPSHOT] MonoTime: " << report.feedback_time.ms() << "ms"
                    ...
                    << " | FinalTargetBps: " << final_target.bps()
