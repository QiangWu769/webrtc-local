...
std::string GetWallClockTimestampString() {
 ...
 void SendSideBandwidthEstimation::SetMinMaxBitrate(DataRate min_bitrate,
                                                    DataRate max_bitrate) {
   DataRate old_min = min_bitrate_configured_;
   DataRate old_max = max_bitrate_configured_;
   ...
   // 添加配置约束日志 (安全处理无穷大值)
   RTC_LOG(LS_INFO) << "[BWE-ConfigLimit] MinBitrate: " << (old_min.IsFinite() ? std::to_string(old_min.bps()) : "INF") 
                    << " -> " << (min_bitrate_configured_.IsFinite() ? std::to_string(min_bitrate_configured_.bps()) : "INF") << " bps"
                    << ", MaxBitrate: " << (old_max.IsFinite() ? std::to_string(old_max.bps()) : "INF") 
                    << " -> " << (max_bitrate_configured_.IsFinite() ? std::to_string(max_bitrate_configured_.bps()) : "INF") << " bps"
 ...
 void SendSideBandwidthEstimation::UpdateReceiverEstimate(Timestamp at_time,
   ...
   // limitation.
   DataRate old_limit = receiver_limit_;
 ...
 void SendSideBandwidthEstimation::UpdateDelayBasedEstimate(Timestamp at_time,
   ...
   // TODO(srte): Ensure caller passes PlusInfinity, not zero, to represent no
   // limitation.
   DataRate old_limit = delay_based_limit_;
 ...
 void SendSideBandwidthEstimation::UpdateLossBasedEstimator(
   ...
   if (!IsInStartPhase(report.feedback_time) && !in_alr) {
 ...
 void SendSideBandwidthEstimation::UpdateEstimate(Timestamp at_time) {
   ...
   if (delay_based_limit_.IsFinite() &&
       delay_based_limit_ < current_target_ &&
       current_target_ > DataRate::Zero()) {
     // This is a heuristic to prevent over-estimating the bandwidth.
     // If the current target is higher than the delay-based estimate,
     // and the delay-based estimate is positive, it means we are
     // currently overusing the bandwidth.
     // We should reduce the target bitrate to the delay-based estimate.
     DataRate new_bitrate = delay_based_limit_;
   ...
   if (rtt_backoff_.IsRttAboveLimit()) {
     TimeDelta time_since_last_decrease = at_time - time_last_decrease_;
     bool can_decrease = time_since_last_decrease >= rtt_backoff_.drop_interval_.Get() &&
                         current_target_ > rtt_backoff_.bandwidth_floor_.Get();
     
     // RTT专属日志 - RTT回退检查
     TimeDelta timeout_correction = std::max(
         rtt_backoff_.last_packet_sent_ - rtt_backoff_.last_propagation_rtt_update_, 
         TimeDelta::Zero());
     ...
     if (can_decrease) {
       ...
       DataRate new_bitrate =
           std::max(current_target_ * rtt_backoff_.drop_fraction_.Get(),
 ...
 void SendSideBandwidthEstimation::UpdateTargetBitrate(DataRate new_bitrate,
                                                       Timestamp at_time) {
   DataRate original_bitrate = new_bitrate;
   DataRate upper_limit = GetUpperLimit();
   
   // 应用上限约束
   new_bitrate = std::min(new_bitrate, upper_limit);
   DataRate after_upper_limit = new_bitrate;
   ...
   RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "]" << " [BWE-ConstraintApply] MonoTime: " << at_time.ms() << " ms"
                    ...
                    << ", MinConfig: " << (min_bitrate_configured_.IsFinite() ? std::to_string(min_bitrate_configured_.bps()) + " bps" : "INF")
                    << ", Final: " << (after_lower_limit.IsFinite() ? std::to_string(after_lower_limit.bps()) + " bps" : "INF")
                    << ", DelayLimit: " << (delay_based_limit_.IsFinite() ? std::to_string(delay_based_limit_.bps()) + " bps" : "INF")
                    << ", ReceiverLimit: " << (receiver_limit_.IsFinite() ? std::to_string(receiver_limit_.bps()) + " bps" : "INF")
                    << ", MaxConfig: " << (max_bitrate_configured_.IsFinite() ? std::to_string(max_bitrate_configured_.bps()) + " bps" : "INF");
