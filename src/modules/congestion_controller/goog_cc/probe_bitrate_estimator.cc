...
std::optional<DataRate> ProbeBitrateEstimator::HandleProbeAndEstimateBitrate(
  ...
  RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "]" << " [ProbeBWE-Success] Cluster ID: " << cluster_id
                   << ", Send rate: " << send_rate.bps() << " bps"
                   << ", Receive rate: " << receive_rate.bps() << " bps"
                   << ", Send: " << ToString(send_size) << " / " << ToString(send_interval)
  ...
  if (receive_rate < kMinRatioForUnsaturatedLink * send_rate) {
    ...
    RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "]" << " [ProbeBWE-Saturated] Cluster ID: " << cluster_id
                     << ", Link appears saturated, adjusting estimate"
                     << ", Original: " << std::min(send_rate, receive_rate).bps() << " bps"
