  ...
  if (calculated_deltas) {
    ...
    switch (hypothesis_) {
      ...
      case BandwidthUsage::kBwOverusing:
        state_str = "Overusing";
        break;
      case BandwidthUsage::kLast:  // 添加这个case
        state_str = "Last";
        break;
    }
    
    RTC_LOG(LS_INFO) << "[" << GetWallClockTimestampString() << "]"
                     << " [Trendline] MonoTime: " << arrival_time_ms
                     << " ms, Delta: " << recv_delta_ms - send_delta_ms
                     << " ms, Accumulated delay: " << accumulated_delay_
                     << " ms, Smoothed delay: " << smoothed_delay_
                     << " ms, Trendline slope: " << prev_trend_
                     << ", Modified trend: " << prev_modified_trend_
