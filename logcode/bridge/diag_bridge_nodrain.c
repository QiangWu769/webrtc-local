...
static int disable_drain = 0;                              // Flag to disable drain thread for testing
...
void start_drain_thread() {
    ...
    // Check if drain is disabled for testing
    if (disable_drain) {
        printf("[+] Drain thread DISABLED for latency comparison test\n");
        return;
    }
...
int main(int argc, char* argv[]) {
    ...
    // Usage: ./diag_bridge_nodrain [buffer_mode] [nodrain]
    // Examples:
    //   ./diag_bridge_nodrain                  # streaming + drain enabled
    //   ./diag_bridge_nodrain streaming        # streaming + drain enabled
    //   ./diag_bridge_nodrain streaming nodrain # streaming + drain DISABLED
    //   ./diag_bridge_nodrain nodrain          # streaming + drain DISABLED

    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "nodrain") == 0) {
            disable_drain = 1;
            printf("[+] Drain thread will be DISABLED (for latency comparison test)\n");
        } else {
            set_buffering_mode(argv[i]);
