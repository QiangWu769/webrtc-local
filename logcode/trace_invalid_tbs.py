...
def trace_invalid_tbs():
    """Add debug logging to track invalid TBS values"""
    
    # Create a test script that will parse with debug output
    test_code = '''
import struct
...
'''
    ...
    with open('test_tbs_debug.py', 'w') as f:
        f.write(test_code)
    
    import subprocess
    subprocess.call(['python', 'test_tbs_debug.py'])
