...
// http://mail.gnome.org/archives/gnome-announce-list/2011-October/msg00041.html
  ...
  const std::string config_file = absl::GetFlag(FLAGS_config);
  if (!config_file.empty()) {
    if (!webrtc::ParseAlphaCCConfig(config_file)) {
  ...
  // AlphaRTC auto close is now handled by video transmission completion detection
  const webrtc::AlphaCCConfig* config = webrtc::GetAlphaCCConfig();
  if (config && config->conn_autoclose > 0) {
    printf("Auto close enabled - will exit when video transmission completes (estimated %d seconds)\n", config->conn_autoclose);
  }
