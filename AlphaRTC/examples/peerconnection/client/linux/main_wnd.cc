...
void GtkMainWnd::StartAutoCloseTimer(int interval_ms) {
  // Fixed: Properly close AlphaRTC by calling callback->Close() and then exit
  // instead of just gtk_main_quit() which doesn't affect the rtc::Thread main loop
  g_timeout_add(interval_ms, [](gpointer data) -> gboolean {
    GtkMainWnd* main_wnd = static_cast<GtkMainWnd*>(data);
    if (main_wnd && main_wnd->GetCallback()) {
      printf("⏰ AlphaRTC auto-close timer triggered, closing connection...\n");
      main_wnd->GetCallback()->Close();
    }
    // Give some time for cleanup, then exit
    g_timeout_add(1000, [](gpointer) -> gboolean {
      printf("✅ AlphaRTC auto-close completed, exiting...\n");
      exit(0);
      return FALSE;
    }, nullptr);
...
#if GTK_MAJOR_VERSION != 2
  // Only draw if we have valid dimensions and buffer
  if (width_ > 0 && height_ > 0 && draw_buffer_.get()) {
...
void GtkMainWnd::VideoRenderer::OnFrame(const webrtc::VideoFrame& video_frame) {
  // Call professional video frame saving callback (same as serverless)
  MainWndCallback* callback = main_wnd_->GetCallback();
  if (callback) {
    callback->OnFrameCallback(video_frame);
  }
