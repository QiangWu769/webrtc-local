...
bool ParseAlphaCCConfig(const std::string& file_path) {
  ...
  if (enabled) {
    ...
    if (enabled) {
      ...
      // Parse bitrate settings (optional, with default values)
      int bitrate_value;
      if (GetInt(third, "max_bitrate_kbps", &bitrate_value) == 0) {
        config->video_max_bitrate_kbps = bitrate_value;
      }
      if (GetInt(third, "min_bitrate_kbps", &bitrate_value) == 0) {
        config->video_min_bitrate_kbps = bitrate_value;
      }
      if (GetInt(third, "start_bitrate_kbps", &bitrate_value) == 0) {
        config->video_start_bitrate_kbps = bitrate_value;
      }
    }
  }
  third.clear();
