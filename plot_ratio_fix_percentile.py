...
# Binned analysis - 0-2范围，使用更大的bin
bins = np.linspace(0, 2, 21)  # 减少到20个bins，每个bin更宽
...
print(f"总数据点: {len(ratios)}")
print(f"有效bins: {len(bin_centers)}")
print(f"\n每个bin的数据点数:")
for i, (bc, count) in enumerate(zip(bin_centers, counts_list)):
    if count > 0:
        print(f"  Bin {i+1} (中心={bc:.3f}): {count} 点")
...
# Plot median and percentiles - 现在所有有效bin都有分位数
ax.plot(bin_centers, medians, 'r-', linewidth=2.5, zorder=3)
ax.fill_between(bin_centers, p25s, p75s, alpha=0.3, color='red', zorder=2)
...
ax.set_xlim(0, 2)
ax.set_ylim(0, max(bitrates.max() * 1.05, medians.max() * 1.05))
...
print(f"\nPlot saved: ratio_vs_bitrate_with_density.png")
