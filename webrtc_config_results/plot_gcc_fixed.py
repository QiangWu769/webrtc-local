...
class GccDecisionAnalyzer:
    ...
    def plot_gcc_decision_metrics(self, data_dict):
        ...
        # Create 11 vertical subplots (original 6 + 3 diag overlays + 2 cellular timing)
        fig, axes = plt.subplots(11, 1, figsize=(18, 44), sharex=True)
        ...
        # 10. Cellular Network Time: SysFN Distribution and Timeline
        if not diag_df.empty and 'sysfn_avg' in diag_df.columns and diag_df['sysfn_avg'].sum() > 0:
            # Primary axis for SysFN values over time  
            axes[9].plot(diag_df['time_s'], diag_df['sysfn_avg'], '-', color='mediumorchid', linewidth=2, alpha=0.8, label='SysFN Avg')
            step = max(1, len(diag_df) // 50)
            axes[9].plot(diag_df['time_s'][::step], diag_df['sysfn_avg'][::step], 'o', color='mediumorchid', markersize=4, alpha=0.9)
            
            # Secondary axis for SubFN values
            ax9_twin = axes[9].twinx()
            ax9_twin.plot(diag_df['time_s'], diag_df['subfn_avg'], '--', color='darkorange', linewidth=1.5, alpha=0.8, label='SubFN Avg')
            ax9_twin.set_ylabel('SubFN (0-9)', fontsize=11, color='darkorange')
            ax9_twin.set_ylim(0, 10)
            ax9_twin.tick_params(axis='y', labelcolor='darkorange')
            
            axes[9].set_ylabel('SysFN (0-1023)', fontsize=11, color='mediumorchid')
            axes[9].set_ylim(0, 1024)
            axes[9].tick_params(axis='y', labelcolor='mediumorchid')
            axes[9].set_title('10. Cellular Network Time: SysFN (10ms) + SubFN (1ms) Distribution', fontsize=12, fontweight='bold')
            ...
            axes[9].text(0.02, 0.95, f'SysFN Range: {sysfn_range} | SubFN Range: {subfn_range}', 
            ...
            axes[9].text(0.5, 0.5, 'No Cellular Timing Data (SysFN/SubFN)', transform=axes[9].transAxes, ha='center', va='center',
                         fontsize=14, alpha=0.5)
            axes[9].set_title('10. Cellular Network Time: SysFN (10ms) + SubFN (1ms) Distribution', fontsize=12, fontweight='bold')
            axes[9].grid(True, alpha=0.3)

        # 11. Cellular Time Precision: Event Order within Same Unix Timestamp
        if not diag_df.empty and 'cellular_time_ms' in diag_df.columns and diag_df['cellular_time_ms'].sum() > 0:
            ...
            axes[10].plot(diag_df['time_s'], diag_df['cellular_time_ms'], '-', color='darkviolet', linewidth=2, alpha=0.8, label='Cellular Time (ms)')
            step = max(1, len(diag_df) // 50)
            axes[10].plot(diag_df['time_s'][::step], diag_df['cellular_time_ms'][::step], 'o', color='darkviolet', markersize=4, alpha=0.9)
            
            axes[10].set_ylabel('Cellular Time (ms)', fontsize=11)
            axes[10].set_title('11. Cellular Time Precision: SysFN×10ms + SubFN×1ms (Event Ordering)', fontsize=12, fontweight='bold')
            ...
            axes[10].text(0.02, 0.95, f'Range: {cellular_range:.0f}ms | Unique Times: {unique_cellular_times}', 
                         transform=axes[10].transAxes, 
                         bbox=dict(boxstyle="round,pad=0.3", facecolor="lightsteelblue", alpha=0.7), fontsize=9)
            axes[10].legend(fontsize=9, loc='upper right')
            
            # Highlight time precision improvement
            axes[10].text(0.98, 0.05, 'This shows precise event timing within same Unix timestamps', 
                         transform=axes[10].transAxes, ha='right', va='bottom', fontsize=8,
                         bbox=dict(boxstyle="round,pad=0.3", facecolor="lightyellow", alpha=0.8), style='italic')
        else:
            axes[10].text(0.5, 0.5, 'No Cellular Time Precision Data', transform=axes[10].transAxes, ha='center', va='center',
                          fontsize=14, alpha=0.5)
            axes[10].set_title('11. Cellular Time Precision: SysFN×10ms + SubFN×1ms (Event Ordering)', fontsize=12, fontweight='bold')
            axes[10].grid(True, alpha=0.3)
